function [estimate_position, RMSE] = TOA_ICPC_static(LS_gauss, r_measure, targ, anchor_errors, gauss_distance)
    % TOA_ICPC_static: Static algorithm for target position estimation using Time of Arrival (TOA) measurements.
    % This function estimates the target position using a two-step optimization process, considering anchor position errors and measurement noise.
    %
    % Inputs:
    ...
    % second step, it refines the target position estimate using the updated anchor positions and their corresponding CRLB values.
    ...
    [~, FIM_inverse] = TOA_local_CRLB(LS_gauss, targ_iCHAN, gauss_distance, anchor_errors);
    ...
    % Second optimization step: Refine the target position estimate
    fitness_function = @(x) calculate_fitness_dsf(x(1:2), x(3:2*(num_anchor+1)), Anchor_estimate_update, r_measure, gauss_distance, CRLB_anchor_update, num_anchor);
    [x, ~] = particleswarm(fitness_function, 2*(num_anchor+1), lower_bound, upper_bound, options);
